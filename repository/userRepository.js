...
const {generateAuthAccessToken, generateAuthRefreshToken } = require('./authRepository')
...
const loginUser = async (data) => {
  try {
    ...
    const accessToken = await generateAuthAccessToken(user);
    const refreshToken = await generateAuthRefreshToken(user);
    // res.send({ user: user.getPublicProfile(), accessToken }) Instead of this we can write below line and make function models/user named toJSON to work exactly like above
